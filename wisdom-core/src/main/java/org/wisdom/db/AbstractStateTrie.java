...
public abstract class AbstractStateTrie<T> implements StateTrie<T>{
    ...
    protected abstract Map<byte[], T> getUpdatedStates(Map<byte[], T> beforeUpdates, Collection<Block> block);
    protected abstract Set<byte[]> getRelatedKeys(Collection<Block> block);
    protected abstract int getBlocksPerUpdate();
